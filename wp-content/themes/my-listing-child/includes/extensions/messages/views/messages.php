...
<script type="text/html" id="ml-conversation">
    <div id="message-inbox-chat" class="message-inbox-chat">
            ...
            <div class="inbox-avatar">
                <span class="msg-listing-info" v-if="isPostAuthor()">
                    <div class="avatar-container">
                        <a :href="chat.postData.link" class="avatar-img" target="_blank">
                            <img :src="chat.postData.image" class="avatar avatar-96 photo" height="96" width="96" />
                        </a>

                        <h6 style="display: block;"><a :href="chat.postData.link" style="display: block;" target="_blank">{{ chat.postData.title }}</a></h6>
                    </div>
                </span>
                <transition-group name="vopacity" v-else>
                    <div class="avatar-container" v-if="!init" :key="'loaded-convo'">
                        <a :href="opponent.uri" class="avatar-img" v-html="opponent.avatar" target="_blank"></a>
                        <div>
                            <h6 style="display: block; "><a :href="opponent.uri" style="display: block;" target="_blank">{{ opponent.name }}</a></h6>
                        </div>
                    </div>
                </transition-group>
            </div>
        ...
        <div class="anuncio-no-chat" v-if="!isPostAuthor()">
            <a :href="chat.postData.link" target="_blank">
                <span><i class="mi open_in_new" style="color: #777;"></i> Anúncio: {{ chat.postData.title }}</span>
            </a>
        </div>
